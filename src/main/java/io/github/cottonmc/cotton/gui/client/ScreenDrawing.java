...
public class ScreenDrawing {
	...
	public static void drawGuiPanel(int x, int y, int width, int height) {
		// TODO: Dark mode
		if (false) drawGuiPanel(x, y, width, height, 0xFF0B0B0B, 0xFF2F2F2F, 0xFF414141, 0xFF000000);
	 ...
	 * @since 1.9.0
	 */
	public static void drawString(MatrixStack matrices, Text text, HorizontalAlignment align, int x, int y, int width, int color) {
		switch(align) {
		case LEFT: {
				MinecraftClient.getInstance().textRenderer.draw(matrices, text.method_30937(), x, y, color);
			}
			break;
		case CENTER: {
				int wid = MinecraftClient.getInstance().textRenderer.getWidth(text);
				int l = (width/2) - (wid/2);
				MinecraftClient.getInstance().textRenderer.draw(matrices, text.method_30937(), x+l, y, color);
			}
			break;
		case RIGHT: {
				int wid = MinecraftClient.getInstance().textRenderer.getWidth(text);
	 ...
	 * @param color    the text color
	 */
	public static void drawStringWithShadow(MatrixStack matrices, Text text, HorizontalAlignment align, int x, int y, int width, int color) {
		switch(align) {
		case LEFT: {
				MinecraftClient.getInstance().textRenderer.drawWithShadow(matrices, text.method_30937(), x, y, color);
			}
			break;
		case CENTER: {
				int wid = MinecraftClient.getInstance().textRenderer.getWidth(text);
				int l = (width/2) - (wid/2);
				MinecraftClient.getInstance().textRenderer.drawWithShadow(matrices, text.method_30937(), x+l, y, color);
			}
			break;
		case RIGHT: {
				int wid = MinecraftClient.getInstance().textRenderer.getWidth(text);
